"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var common_1 = require("@nestjs/common");
var fs = require('fs');
var CancionService = /** @class */ (function () {
    function CancionService() {
        this.NOMBRE_BD = 'C:\\Users\\analy.chinacalle\\Documents\\GitHub\\Chinacalle-Paredes-Analy-Belen\\03-http\\deberHTTP\\webregistro\\canciones.json';
    }
    CancionService.prototype.listar = function () {
        return __awaiter(this, void 0, void 0, function () {
            var promesaLeer, bdd, e_1;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        promesaLeer = function () {
                            // @ts-ignore
                            return new Promise(function (resolve) {
                                fs.readFile(_this.NOMBRE_BD, 'utf-8', function (error, contenidoLeido) {
                                    if (error) {
                                        resolve(null);
                                    }
                                    else {
                                        resolve(JSON.parse(contenidoLeido));
                                    }
                                });
                            });
                        };
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, promesaLeer()];
                    case 2:
                        bdd = _a.sent();
                        return [2 /*return*/, promesaLeer()];
                    case 3:
                        e_1 = _a.sent();
                        console.error(e_1);
                        throw new common_1.InternalServerErrorException({ mensaje: 'Error servidor' });
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    CancionService.prototype.crear = function (cancion) {
        return __awaiter(this, void 0, void 0, function () {
            var promesaLeer, bdd, ultimoIndice, e_2;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        promesaLeer = function () {
                            // @ts-ignore
                            return new Promise(function (resolve) {
                                fs.readFile(_this.NOMBRE_BD, 'utf-8', function (error, contenidoLeido) {
                                    if (error) {
                                        resolve(null);
                                    }
                                    else {
                                        resolve(JSON.parse(contenidoLeido));
                                    }
                                });
                            });
                        };
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, promesaLeer()];
                    case 2:
                        bdd = _a.sent();
                        ultimoIndice = bdd.canciones.length;
                        console.log(JSON.stringify(bdd));
                        cancion.id = ultimoIndice + 1;
                        cancion.autor.id = ultimoIndice + 1;
                        bdd.canciones.push(cancion);
                        //return promesaLeer();
                        return [2 /*return*/, guardarBase(bdd)];
                    case 3:
                        e_2 = _a.sent();
                        console.error(e_2);
                        throw new common_1.InternalServerErrorException({ mensaje: 'Error servidor' });
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    CancionService.prototype.eliminar = function (nombreCancion) {
        return __awaiter(this, void 0, void 0, function () {
            var promesaLeer, base, contenido, bdd, indiceCancion, e_3;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        promesaLeer = function () {
                            // @ts-ignore
                            return new Promise(function (resolve) {
                                fs.readFile(_this.NOMBRE_BD, 'utf-8', function (error, contenidoLeido) {
                                    if (error) {
                                        resolve(null);
                                    }
                                    else {
                                        resolve(JSON.parse(contenidoLeido));
                                    }
                                });
                            });
                        };
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, promesaLeer()];
                    case 2:
                        base = _a.sent();
                        contenido = JSON.stringify(base);
                        bdd = JSON.parse(contenido);
                        indiceCancion = buscarCancion(nombreCancion, bdd);
                        console.log('indice' + indiceCancion);
                        bdd.canciones
                            .splice(indiceCancion, 1);
                        console.log(JSON.stringify(bdd));
                        return [2 /*return*/, guardarBase(bdd)];
                    case 3:
                        e_3 = _a.sent();
                        console.error(e_3);
                        throw new common_1.InternalServerErrorException({ mensaje: 'Error servidor' });
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    CancionService.prototype.actualizar = function (nombreCancion, nuevaCancion) {
        return __awaiter(this, void 0, void 0, function () {
            var promesaLeer, baseCanciones, contenido, baseC, indiceCancion, e_4;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        promesaLeer = function () {
                            // @ts-ignore
                            return new Promise(function (resolve) {
                                fs.readFile(_this.NOMBRE_BD, 'utf-8', function (error, contenidoLeido) {
                                    if (error) {
                                        resolve(null);
                                    }
                                    else {
                                        resolve(JSON.parse(contenidoLeido));
                                    }
                                });
                            });
                        };
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, promesaLeer()];
                    case 2:
                        baseCanciones = _a.sent();
                        contenido = JSON.stringify(baseCanciones);
                        baseC = JSON.parse(contenido);
                        indiceCancion = buscarCancion(nombreCancion, baseC);
                        console.log('anti' + baseC.canciones[indiceCancion]);
                        console.log('nueva: ' + nuevaCancion);
                        baseC.canciones[indiceCancion] = nuevaCancion;
                        console.log(JSON.stringify(baseC));
                        return [2 /*return*/, guardarBase(baseC)];
                    case 3:
                        e_4 = _a.sent();
                        console.error(e_4);
                        throw new common_1.InternalServerErrorException({ mensaje: 'Error servidor' });
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    CancionService = __decorate([
        common_1.Injectable()
    ], CancionService);
    return CancionService;
}());
exports.CancionService = CancionService;
function buscarCancion(nombre, bdd) {
    var contenidoActual = JSON.stringify(bdd);
    var baseActual = JSON.parse(contenidoActual);
    var indiceDeCancion = baseActual.canciones
        .findIndex(function (cancion) {
        return cancion.nombre === nombre;
    });
    return indiceDeCancion;
}
function guardarBase(bdd) {
    var _this = this;
    // @ts-ignore
    return new Promise(function (resolve, reject) {
        fs.writeFile(_this.NOMBRE_BD, JSON.stringify(bdd, null, 2), function (error) {
            if (error) {
                reject({ Mensaje: 'error guardando', error: 500 });
            }
            else {
                resolve({ Mensaje: 'Base guardada' });
            }
        });
    });
}
